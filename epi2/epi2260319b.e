...
{
    /*baige addRF*/
    a_rftrk    =opfliptrk/180.0;
    thk_rftrk = opslthick; 
    res_rftrk  = 320;
    flip_rftrk = opfliptrk;
    pw_rftrk   = 3200;   
/*baige addRF end*/
...
/*baige addRF*/
  ...
  gradx[GXW2TRK_SLOT].powscale = 1;
    ...
    /* baige: force tracking-related gradient instruction amplitudes to 0 in predownload */
    ia_gzrftrk = 0;
    ia_gz2 = 0;
    ia_gzktrk = 0;
    ia_gxktrk = 0;
    fprintf(stderr, "[DBG] predownload force ia_gzrftrk=%d ia_gz2=%d ia_gzktrk=%d ia_gxktrk=%d\n",
                    ia_gzrftrk, ia_gz2, ia_gzktrk, ia_gxktrk);
    fflush(stderr);
  /*baige addRF end*/
    return SUCCESS;
}    /* end predownload1() */
...
fflush(stdout);
            ...
            /* Data Acquisition */
        ACQUIREDATA(echo1, pbeg( &gxwtrk, "gxwtrk", 0 ), , , );
    /* baige addGx end */
     /* Z & X Killers */
    /* ---- Debug gzktrk params ---- */
    #if defined(HOST_TGT)
...
@inline HoecCorr.e HoecUpdateReadoutBlipAmpRsp
                    ...
                    if ((rspent == L_SCAN) && (opdiffuse == PSD_ON) && (use_sl == 1))
                    {
                        ...
                        loaddab(&echo1, 0, 0, dabop, 0, DABON, PSD_LOAD_DAB_ALL);

                        startseq((short)sliceindex1, (SHORT)MAY_PAUSE);
                        syncoff(&seqtrk);

                        /* Restore imaging receive/DAB state after using echo1 in seqtrk. */
                        setfrequency(sl_rcvcf, &echo1, 0);
                        dabop = dabop_saved;
                    ...
                    fprintf(stderr,"[PARAMS] IMAGING(start) PARAMS:  pass_rep=%d, pass=%d, core_rep=%d, ileave=%d, excitation=%d, false_pass=%d, false_slice=%d\n",pass_rep, pass, core_rep, ileave, excitation, false_pass, false_slice);
                    fflush(stderr);

                    boffset(off_seqcore);
                    startseq((short)sliceindex1, (SHORT)pause);
                    fprintf(stderr, "[DBG] IMAGING(end): pass=%d diff_index=%d sliceindex1=%d pause=%d\n",
                                     pass, diff_index, sliceindex1, pause);
                    fflush(stderr);
