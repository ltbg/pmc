...
/***************************************************
  ...
  7T   11/05/2020 GW   HCSDM00633697: Removed checking the delay.esph.xyz file.
...
STATUS epiCalFileCVCheck (void) {
    ...
    nobcfile = 0;
    for( recvnum = (cfrecvst + 1); recvnum < (int)(getRxNumChannels() + 1); recvnum++ ) {
    ...
    number_of_bc_files = (int)getRxNumChannels() - nobcfile;
    if(activeReceivers != (int)getRxNumChannels()) {
        activeReceivers = (int)getRxNumChannels(); 
        flagWarning = TRUE;
    }


    if ( ((int)getRxNumChannels() > number_of_bc_files) && (exist(opepi)==PSD_ON) ) {
