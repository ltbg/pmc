/**
*这个版本是加了RF grad switch正确运行版本 simulation通过 hardware DAB错误
...
@inline HoecCorr.e HoecUpdateReadoutBlipAmpRsp
                    ...
                    if ((rspent == L_SCAN) && (opdiffuse == PSD_ON) && (use_sl == 1))
                    {
                        ...
                        {
                            fprintf(stderr,"[DBG] TRK_CHECK(%d): diff_index=%d ioffset=%d is_diff=%d b=%d dir=%d last_b=%d last_dir=%d false_slice=%d rspslqb=%d\n",
                                    dbg_trk_check_count,
                                    diff_index,
                                    ioffset,
                                    local_is_diff_frame,
