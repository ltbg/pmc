...
/* baige addRF */
    /* Tracking 序列：独立 RF + 简单读出 */
    SLICESELZ(rftrk, 1ms, 3200us, opslthick, opflip, 1, , loggrd);  /* 后续非选层可改宏 */

    /* Reuse existing readout gradient gxw to avoid adding a new X slot */
    /* Timing still anchored to gzrftrk if slice-select tracking is used */
    TRAPEZOID(XGRAD, gxw,
        RUP_GRD(pmid(&gzrftrk,"gzrftrk",0) + opte - pw_gxw/2),
        0, TYPNDEF, loggrd);

    ACQUIREDATA(echo_trk, pbeg(&gxw,"gxw",0), , , );

    SEQLENGTH(seqtrk, optr, seqtrk);   /* tracking 序列长度 */
/* baige addRF */
...
/* baige addRF */
    setrfltrs( (int)filter_echo1, &echo_trk );   /* 复用滤波 tracking 的 BW/读出时长以后不同，
    再另外 calcfilter/setrfltrs 即可 */
...
scan( void )
{
    ...
    setssitime( time_ssi );	/* allow time to update sequencer memory */
...
/* baige addRF: 每两个 imaging 后一个 tracking（view%3==0） */
                if( (view % 3) == 0 )
                {
                    ...
                    continue; /* 本次 excitation 已完成，进入下一次 */
                }
/* baige addRF end */
...
{   
    if (isrtplaunched)
    {
        if (RtpEnd() != SUCCESS)
        {
            return EM_PSD_RTP_CLEANUP_FAILED;
        }

        isrtplaunched = 0;
    }

    return EM_PSD_NO_ERROR;
} 
