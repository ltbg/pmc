/*
 *grass260317b 是在grass260317b的基础上 
 * 之前ia参数没有修改成功 需要在cv自己改才可以tracking
 * 修改为predowload ia参数直接设置为0
* 写死opflip和optr
 * GE Medical Systems
...
/* baige addRF */
    /* Tracking 序列 */
    SLICESELZ(rftrk, 1ms, 3200us, opslthick,10, 1, , loggrd);
...
scan( void )
{
    ...
    for( slice = 0; slice < opslquant; ++slice )
    {
        ...
        for( view = 1; view < (opyres + 1); ++view )
        {
            ...
            {
/* baige addRF: 每隔10个imaging插入3个tracking。
 * 周期长度=13：前10个view做imaging，后3个view做tracking。 */
                if( ((view - 1) % 13) >= 10 )
                {
                    ...
                    setiampt(0, &gz2, 0);
                    fprintf( stderr, "=====after setiampt(0, &gz2, 0)=====\n" );
                    fflush(stderr);
